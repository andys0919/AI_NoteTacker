...
const AbstractMeetBot_1 = require("./AbstractMeetBot");
const config_1 = __importDefault(require("../config"));
const error_1 = require("../error");
const uuid_1 = require("uuid");
const botService_1 = require("../services/botService");
const RecordingTask_1 = require("../tasks/RecordingTask");
const ContextBridgeTask_1 = require("../tasks/ContextBridgeTask");
...
class BotBase extends AbstractMeetBot_1.AbstractMeetBot {
    constructor(logger, correlationId) {
        super();
        this.slightlySecretId = Symbol((0, uuid_1.v4)());
        this._logger = logger;
...
class ZoomBot extends BotBase {
    ...
    async joinMeeting({ pushState, ...params }) {
        ...
        try {
            ...
            await (0, bugService_1.uploadDebugImage)(await this.page.screenshot({ type: 'png', fullPage: true }), 'accept-cookie', params.userId, this._logger, params.botId);
        ...
        const findAndEnableJoinFromBrowserButton = async (retry) => {
            try {
                if (retry >= attempts) {
                    return false;
                }
        ...
        const visitWebClientByUrl = async () => {
        ...
        const waitForJoinFromBrowserNav = async () => {
            try {
                const maxAttempts = 3;
                let attempt = 0;
                const navPromise = new Promise((foundResolver) => {
                    const interv = setInterval(async () => {
                        ...
                        try {
                            const joinFromBrowser = await this.page.locator('a', { hasText: 'Join from your browser' }).first();
                            await joinFromBrowser.waitFor({ timeout: 4000 }).catch(() => undefined);
                            if (await joinFromBrowser.count() > 0) {
        ...
        if (!foundAndClickedJoinFromBrowser || !navSuccess) {
            await (0, bugService_1.uploadDebugImage)(await this.page.screenshot({ type: 'png', fullPage: true }), 'enable-join-from-browser', params.userId, this._logger, params.botId);
            ...
            if (!canAccess) {
                await (0, bugService_1.uploadDebugImage)(await this.page.screenshot({ type: 'png', fullPage: true }), 'direct-access-webclient', params.userId, this._logger, params.botId);
        ...
        let iframe = this.page;
        const apps = [];
        const detectAppContainer = async (startWith) => {
            try {
                ...
                await (0, bugService_1.uploadDebugImage)(await this.page.screenshot({ type: 'png', fullPage: true }), 'detect-app-container', params.userId, this._logger, params.botId);
        ...
        const reportJoinRequestSubmitted = async (bodyText) => {
            ...
            await (0, botService_1.addBotLog)({
                level: 'info',
                message: 'Zoom join request was submitted and is waiting for host admission.',
                provider: 'zoom',
                token: bearerToken,
                botId: params.botId,
        ...
        try {
            ...
            const waitAtLobbyPromise = new Promise((resolveMe) => {
                ...
                waitInterval = setInterval(async () => {
                    try {
                        ...
                        // Keep waiting until waiting-room or admitted state becomes clear.
                    }
                }, 2000);
            });
            ...
            if (!joined) {
                ...
                throw new error_1.WaitingAtLobbyRetryError('Zoom bot could not enter the meeting...', bodyText ?? '', false, 0);
            }
        ...
        let notifyTimeout;
        try {
            const cameraNotifications = [];
            const micNotifications = [];
            ...
            const notifyPromise = new Promise((res) => {
                ...
                notifyInternval = setInterval(async () => {
                    try {
                        ...
                        else if (cameraNotifications.includes('found')) {
                        ...
                        else if (micNotifications.includes('found')) {
        ...
        await (0, botService_1.addBotLog)({
            level: 'info',
            message: 'Zoom bot joined the meeting and started recording.',
            provider: 'zoom',
            token: bearerToken,
            botId,
            eventId,
    ...
    async recordMeetingPage(params) {
        ...
        const chores = new ContextBridgeTask_1.ContextBridgeTask(this.page, { ...params, botId: params.botId ?? '' }, this.slightlySecretId.toString(), waitingPromise, uploader, this._logger);
...
//# sourceMappingURL=ZoomBot.js.map
