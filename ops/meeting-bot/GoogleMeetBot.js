...
const promise_1 = require("../lib/promise");
const resilience_1 = require("../util/resilience");
const bugService_1 = require("../services/bugService");
const chromium_1 = __importDefault(require("../lib/chromium"));
const constants_1 = require("../constants");
const recording_1 = require("../lib/recording");
const { detectJoinRequestEvidence } = require("./join_request_evidence.cjs");
class GoogleMeetBot extends MeetBotBase_1.MeetBotBase {
    constructor(logger, correlationId) {
        super();
        this.slightlySecretId = (0, uuid_1.v4)();
        this._logger = logger;
    ...
    async join({ url, name, bearerToken, teamId, timezone, userId, eventId, botId, uploader }) {
        ...
        try {
            ...
            if (error instanceof error_1.UnsupportedMeetingError) {
                await (0, MeetBotBase_1.handleUnsupportedMeetingError)({ token: bearerToken, botId, eventId, provider: 'google', error }, this._logger);
            }
            throw error;
        }
    }
    async joinMeeting({ url, name, bearerToken, teamId, userId, eventId, botId, pushState, uploader }) {
        ...
        this.page = await (0, chromium_1.default)(url, this._correlationId, 'google');
        ...
        const verifyItIsOnGoogleMeetPage = async () => {
            try {
        ...
        if (googleMeetPageStatus === 'SIGN_IN_PAGE') {
            ...
            throw new error_1.UnsupportedMeetingError('Meeting requires sign in', googleMeetPageStatus);
        }
        ...
        await (0, resilience_1.retryActionWithWait)('Waiting for the input field', async () => await this.page.waitForSelector('input[type="text"][aria-label="Your name"]', { timeout: 10000 }), this._logger, 3, 15000, async () => {
            await (0, bugService_1.uploadDebugImage)(await this.page.screenshot({ type: 'png', fullPage: true }), 'text-input-field-wait', userId, this._logger, botId);
        });
        this._logger.info('Waiting for 10 seconds...');
        await this.page.waitForTimeout(10000);
        ...
        this._logger.info('Waiting for 10 seconds...');
        await this.page.waitForTimeout(10000);
        await (0, resilience_1.retryActionWithWait)('Clicking the "Ask to join" button', async () => {
            ...
            await (0, bugService_1.uploadDebugImage)(await this.page.screenshot({ type: 'png', fullPage: true }), 'ask-to-join-button-click', userId, this._logger, botId);
        });
        ...
        for (let joinEvidenceAttempt = 0; joinEvidenceAttempt < 5 && !joinRequestReported; joinEvidenceAttempt++) {
            await this.page.waitForTimeout(1000);
            const currentBodyText = await this.page.evaluate(() => document.body.innerText).catch(() => '');
            await reportJoinRequestSubmitted(currentBodyText);
        }
        // Do this to ensure meeting bot has joined the meeting
        try {
            const wanderingTime = config_1.default.joinWaitTime * 60 * 1000; // Give some time to admit the bot
            let waitTimeout;
            let waitInterval;
            const waitAtLobbyPromise = new Promise((resolveWaiting) => {
                ...
                waitInterval = setInterval(async () => {
                    try {
                        ...
                        await reportJoinRequestSubmitted(currentBodyText);
                        if (evidence.status === 'denied' || evidence.status === 'timed-out') {
                            this._logger.info('Google Meet Bot could not remain in the lobby waiting state...', { userId, teamId, status: evidence.status });
                            clearInterval(waitInterval);
                            clearTimeout(waitTimeout);
                            resolveWaiting(false);
                            return;
                        }
                        const detectLobbyModeHostWaitingText = async () => {
                            try {
                                const lobbyModeHostWaitingText = await this.page.getByText(constants_1.GOOGLE_LOBBY_MODE_HOST_TEXT);
                                if (await lobbyModeHostWaitingText.count() > 0 && await lobbyModeHostWaitingText.isVisible()) {
                                    return 'WAITING_FOR_HOST_TO_ADMIT_BOT';
                                }
                                const lobbyModeRequestTimeoutText = await this.page.getByText(constants_1.GOOGLE_REQUEST_TIMEOUT);
                        ...
                        try {
                            const deniedText = await this.page.getByText(constants_1.GOOGLE_REQUEST_DENIED);
            ...
            if (!waitingAtLobbySuccess) {
                ...
                throw new error_1.WaitingAtLobbyRetryError('Google Meet bot could not enter the meeting...', bodyText ?? '', false, 0);
            }
        }
        catch (lobbyError) {
    ...
    async recordMeetingPage({ teamId, userId, eventId, botId, uploader }) {
        const duration = config_1.default.maxRecordingDuration * 60 * 1000;
        const inactivityLimit = config_1.default.inactivityLimit * 60 * 1000;
        // Capture and send the browser console logs to Node.js context
        this.page?.on('console', async (msg) => {
            try {
                await (0, logger_1.browserLogCaptureCallback)(this._logger, msg);
            }
            catch (err) {
        ...
        await this.page.exposeFunction('screenAppSendData', async (slightlySecretId, data) => {
            if (slightlySecretId !== this.slightlySecretId)
                return;
        ...
        await this.page.evaluate(async ({ teamId, duration, inactivityLimit, userId, slightlySecretId, activateInactivityDetectionAfter, activateInactivityDetectionAfterMinutes, primaryMimeType, secondaryMimeType }) => {
            let timeoutId;
            let inactivityParticipantDetectionTimeout;
            let inactivitySilenceDetectionTimeout;
            let isOnValidGoogleMeetPageInterval;
            ...
            async function startRecording() {
                ...
                let dismissModalsInterval;
                let lastDimissError = null;
                    ...
                    function getContributorsCount() {
                        function findPeopleButton() {
                            try {
                                ...
                                btn = allBtnsWithLabel.find(b => Array.from(b.querySelectorAll('i')).some(i => i.textContent && i.textContent.trim() === 'people'));
                                if (btn)
                                    return btn;
                                // 6. Not found
                                return null;
                        ...
                        try {
                            ...
                            if (peopleBtn) {
                                ...
                                for (const searchRoot of searchRoots) {
                                    ...
                                    const badgeDiv = searchRoot.querySelector('div.egzc7c');
                                    if (badgeDiv) {
                ...
                const detectMeetingIsOnAValidPage = () => {
                    const googleMeetRecordingStateGracePeriodMs = 60000;
                    const positiveMeetingTokens = ['Meeting tools', 'Chat with everyone', 'Press Down Arrow to open', 'Presenting', 'Meeting details', '會議工具', '與所有人聊天', '會議詳細資料', '發言', '參與者'];
                    const negativeMeetingTokens = ["You've been removed from the meeting", 'No one responded to your request to join the call', '你無法加入這場視訊通話', '任何人都必須經過主辦人邀請或允許，才能加入會議'];
                    const knownButtonLabelFragments = ['people', 'leave call', 'leave meeting', 'meeting details', 'participants', '參與者', '離開通話', '離開會議', '會議詳細資料'];
                    ...
                    const isOnValidGoogleMeetPage = () => {
                        try {
                            ...
                            if (negativeMeetingTokens.some((token) => currentBodyText.includes(token))) {
                                console.warn('Google Meet reported an explicit leave or admission failure state on team:', userId, teamId);
                                return false;
                            }
                            const visibleButtons = Array.from(document.querySelectorAll('button[aria-label]')).filter((button) => button && button.offsetParent !== null);
                            const visibleButtonAriaLabels = visibleButtons.map((button) => button.getAttribute('aria-label') || '').filter((label) => label.length > 0);
                            const hasAvatarCountBadge = document.querySelector('[data-avatar-count]') !== null;
                            const hasKnownButtonLabels = visibleButtonAriaLabels.some((label) => {
                                const normalizedLabel = label.toLowerCase();
                                return knownButtonLabelFragments.some((fragment) => normalizedLabel.includes(fragment));
                            });
                            const hasKnownMeetingText = positiveMeetingTokens.some((token) => currentBodyText.includes(token));
                            if (!hasAvatarCountBadge && !hasKnownButtonLabels && !hasKnownMeetingText) {
                                if (Date.now() - recordingStartedAtMs < googleMeetRecordingStateGracePeriodMs) {
                                    console.warn('Google Meet UI elements not found during startup grace period, continuing recording.');
                                    return true;
                                }
            ...
            activateInactivityDetectionAfterMinutes: config_1.default.activateInactivityDetectionAfter,
            activateInactivityDetectionAfter: new Date(new Date().getTime() + config_1.default.activateInactivityDetectionAfter * 60 * 1000).toISOString(),
            primaryMimeType: recording_1.webmMimeType,
            secondaryMimeType: recording_1.vp9MimeType
        });
        ...
        const waitingPromise = (0, promise_1.getWaitingPromise)(processingTime + duration);
        // Handle graceful shutdown signals so the recording can be finalized
        const handleStopSignal = () => {
            this._logger.info('Process stop requested, finalizing current Google Meet recording...');
            waitingPromise.resolveEarly();
        };
...
//# sourceMappingURL=GoogleMeetBot.js.map
