...
### Requirement: Luna high is used only for meeting summary generation

The system SHALL use `gpt-5.6-luna` with `reasoning.effort=high` only in the
...
- **THEN** the summary worker sends the display transcript to a new
  `gpt-5.6-luna` request with `reasoning.effort=high`
